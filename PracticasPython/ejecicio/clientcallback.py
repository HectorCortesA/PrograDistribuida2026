...
##IP LOCAL
callback_server = SimpleXMLRPCServer(("172.26.160.169", 9000), allow_none=True, logRequests=False)
...
# Cliente RPC
## IP del servidor que tiene implementada la función longtask
server = xmlrpc.client.ServerProxy("http://172.26.162.170:8000/", allow_none=True)

def send_request():
    """Función que envía la solicitud al servidor en un hilo separado."""
    print("Solicitud enviada al servidor, esperando respuesta...")
    try:
        server.long_task("http://172.26.160.169:9000/")  # Llamada RPC con callback para mandar IP LOCAL
